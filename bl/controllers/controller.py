...
class Controller(Generic[T]):
    ...
    async def delete(self, id: str) -> List[T]:
        return await self.db_handler.delete_or_abort(id)

    async def get(self, id: str) -> List[T]:
        return await self.db_handler.get(id)

    async def put(self, id: str, obj_data: T) -> List[T]:
        return await self.db_handler.put(id, obj_data)

    async def post(self, obj_data: T) -> List[T]:
